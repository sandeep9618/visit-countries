...
class App extends Component {
  ...
  onVisitCountry = id => {
    ...
    const filterData = countriesList.map(i => {
      if (i.id === id) {
        return {...i, isVisited: true}
      }
      return i
    })
    this.setState({countriesList: filterData})
  }
  ...
  render() {
    const {countriesList} = this.state
    const visitedCountries = countriesList.filter(i => i.isVisited)
    return (
          ...
          <ul className="visited-countries-container">
            {visitedCountries.map(eachItem => (
              <VisitedCountryItem
                eachItem={eachItem}
                key={eachItem.id}
                onRemoveVisitedState={this.onRemoveVisitedState}
              />
            ))}
          </ul>
        )}
